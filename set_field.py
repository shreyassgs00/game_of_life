...
class Field: #A field object which contains a method to form the field array with which the processing is done.
...
def build_field(field_array):  #Function to display the array in a series of 1's and 0's as a field. 
    for i in range (0,len(field_array)):
        for j in range(0,len(field_array[i])):
            print(field_array[i][j],end='')
        print()

def place_random_ones(field_array):  #A function to place random ones (pseduo-random) in the field array generated by the previous class method. 
    random_places = []
    for i in range (0,len(field_array)):
